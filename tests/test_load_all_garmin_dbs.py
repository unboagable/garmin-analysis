...
class TestSummarizeAndMerge:
    ...
    @pytest.mark.integration
    def test_lag_feature_shift(self, tmp_db):
        df = summarize_and_merge(return_df=True)
    ...
    @pytest.mark.integration
    def test_column_dtypes(self, tmp_db):
        df = summarize_and_merge(return_df=True)
    ...
    @pytest.mark.integration
    def test_dates_valid(self, tmp_db):
        df = summarize_and_merge(return_df=True)
...
class TestDataIngestionHelpers:
    ...
    def test_preprocess_sleep(self):
        sleep = pd.DataFrame({
            "day": ["2024-01-01"],
            "total_sleep": ["7:00:00"],
            "deep_sleep": ["1:00:00"],
    ...
    def test_aggregate_stress(self):
        df = pd.DataFrame({
