...
class TestUpdateDataQualityCharts:
    ...
    def test_empty_dq_returns_empty_figures_and_message(self, tmp_path, monkeypatch):
        ...
        timeline, dist, scatter, stats = update_data_quality_charts(None, None, 0)
    ...
    def test_with_dq_data_returns_figures(self, tmp_path, monkeypatch):
        ...
        timeline, dist, scatter, stats = update_data_quality_charts("2024-01-01", "2024-01-10", 0)
        assert timeline is not None
    ...
    def test_date_filter_narrows_data(self, tmp_path, monkeypatch):
        ...
        timeline, dist, scatter, stats = update_data_quality_charts("2024-01-05", "2024-01-10", 0)
        assert timeline is not None
        assert len(timeline.data[0].x) == 6


class TestUpdateCoverageCharts:
    """Tests for 24-hour coverage analysis callback."""

    def test_object_dtype_has_24h_coverage_does_not_error(self, tmp_path, monkeypatch):
        """Regression: object bool column from CSV must not break monthly aggregation."""
        master_df = pd.DataFrame(
            {
                "day": pd.date_range("2024-01-01", periods=3, freq="D"),
                "coverage_hours": [20.0, 24.0, 18.0],
                "coverage_pct": [83.3, 100.0, 75.0],
                "has_24h_coverage": [False, True, False],
            }
        )
        ...
        timeline, pct, heatmap, stats = update_coverage_charts("2024-01-01", "2024-01-03")
        assert "Error loading coverage data" not in (timeline.layout.title.text or "")
        assert timeline is not None
