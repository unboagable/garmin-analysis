...
def filter_by_date(df, date_col="day", from_date=None, to_date=None, days_back=None, weeks_back=None, months_back=None):
    ...
    if days_back:
        ...
        from_date = now - pd.DateOffset(months=months_back)
...
# Optional: Apply date filter (e.g., last 60 days)
daily = filter_by_date(daily, days_back=60)
sleep = filter_by_date(sleep, days_back=60)
stress = filter_by_date(stress, date_col="timestamp", days_back=60)
rest_hr = filter_by_date(rest_hr, days_back=60)
