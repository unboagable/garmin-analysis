...
from garmin_analysis.features.coverage import (
    ...
    filter_by_24h_coverage,
    load_monitoring_hr_for_coverage,
)


class TestLoadMonitoringHrForCoverage:
    def test_returns_preloaded_hr_filtered(self):
        hr_df = pd.DataFrame(
            {
        ...
        result = load_monitoring_hr_for_coverage(hr_df=hr_df)
        assert len(result) == 1
        assert result["heart_rate"].iloc[0] == 70

    def test_empty_preloaded_returns_none(self):
        assert load_monitoring_hr_for_coverage(hr_df=pd.DataFrame()) is None
...
class TestFilterBy24hCoverage:
    ...
    def test_no_hr_data(self):
        ...
        df = pd.DataFrame(
            {"day": pd.date_range("2024-01-01", periods=5), "steps": [1000, 2000, 3000, 4000, 5000]}
        )
    ...
    def test_hr_data(self):
        """Test filtering with pre-loaded HR data (heart rate indicates watch wear)"""
        master_df = pd.DataFrame(
            {"day": pd.date_range("2024-01-01", periods=3), "steps": [1000, 2000, 3000]}
        )

        start_time = pd.Timestamp("2024-01-01 00:00:00")
        timestamps = [start_time + timedelta(minutes=i) for i in range(1440)]

        hr_df = pd.DataFrame({"timestamp": timestamps, "heart_rate": [70] * len(timestamps)})
    ...
    def test_total_allowance(self):
        ...
        master_df = pd.DataFrame(
            {"day": pd.date_range("2024-01-01", periods=2), "steps": [1000, 2000]}
        )

        day = pd.Timestamp("2024-01-01")
        ts1a = pd.date_range(day, day + pd.Timedelta(hours=6), freq="1min", inclusive="left")
        ts1b = pd.date_range(
            day + pd.Timedelta(hours=6, minutes=15),
        ...
        hr_df = pd.DataFrame({"timestamp": hr_timestamps, "heart_rate": [70] * len(hr_timestamps)})
...
class TestDaysWithContinuousCoverage:
    def test_full_day(self):
        ...
        stress_df = pd.DataFrame({"timestamp": timestamps, "stress_level": [50] * len(timestamps)})
    ...
    def test_allowance_edge_deficit(self):
        ...
        ts = pd.date_range(
            ...
            day + pd.Timedelta(days=1) - pd.Timedelta(minutes=10),
            freq="1min",
        )
