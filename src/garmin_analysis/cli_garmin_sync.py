...
For more information, see: https://github.com/tcgoetz/GarminDB
    ...
    action_group.add_argument(
        '--sync',
        action='store_true',
        help='Sync data from Garmin Connect'
    )
    action_group.add_argument(
        '--backup',
        action='store_true',
        help='Backup GarminDB databases'
    )
    action_group.add_argument(
        '--stats',
        action='store_true',
        help='Show statistics about synced data'
    )
    action_group.add_argument(
        '--copy-dbs',
        action='store_true',
    ...
    action_group.add_argument(
        '--find-dbs',
        action='store_true',
        help='Find and list GarminDB databases (without copying)'
    )
    ...
    setup_group.add_argument(
        '--username',
        help='Garmin Connect username/email (required for --setup)'
    )
    setup_group.add_argument(
        '--password',
        help='Garmin Connect password (required for --setup)'
    )
    setup_group.add_argument(
        '--start-date',
    ...
    setup_group.add_argument(
        '--end-date',
        help='End date for data download in MM/DD/YYYY format (default: today)'
    )
    setup_group.add_argument(
    ...
    sync_group.add_argument(
        '--latest',
        action='store_true',
        help='Only download latest data (for daily updates)'
    )
    sync_group.add_argument(
        '--all',
        dest='all_data',
    ...
    sync_group.add_argument(
        '--no-import',
        action='store_true',
        help='Skip import step (only download)'
    )
    sync_group.add_argument(
        '--no-analyze',
        action='store_true',
        help='Skip analyze step (only download and import)'
    )
    ...
    try:
        ...
        # Handle sync
            ...
            if not GARMINDB_CONFIG_FILE.exists():
                logger.error(f"GarminDB not configured!")
                logger.error(f"Run setup first: {sys.argv[0]} --setup --username your@email.com --password yourpassword")
                return 1
            ...
            if result['success']:
                ...
                logger.info("\nNext steps:")
                logger.info("  1. Generate unified dataset: poetry run python -m garmin_analysis.data_ingestion.load_all_garmin_dbs")
        ...
        # Handle find-dbs
            ...
            if dbs:
                ...
                logger.info("Run GarminDB first: garmindb_cli.py --all --download --import --analyze")
                return 1
        ...
        # Handle copy-dbs
            ...
            if result['copied']:
                ...
                logger.info("  poetry run python -m garmin_analysis.data_ingestion.load_all_garmin_dbs")
                return 0
            elif result['errors']:
