...
class GarminDataQualityAnalyzer:
    ...
    def analyze_garmin_data(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            # 24h continuous coverage (based on monitoring_hr heart rate if available)
            try:
                ...
                if hr is not None and not hr.empty:
                    full_days = days_with_continuous_coverage(hr, timestamp_col="timestamp")
                    self.analysis_results["continuous_24h_days"] = [
                        str(d.date()) for d in full_days
                    ]
    ...
    def _analyze_completeness(self, df: pd.DataFrame) -> dict:
        ...
        for col in df.columns:
            ...
            completeness_data[col] = {
                ...
                "is_sufficient_for_modeling": bool(
                    non_null_count >= 50
    ...
    def _identify_quality_issues(self, df: pd.DataFrame) -> dict:
        ...
        for col in df.columns:
                ...
                if len(non_null) > 0:
                    ...
                    if len(type_counts) > 1:
                        issues["mixed_types"].append(
                            {"column": col, "type_distribution": type_counts.to_dict()}
                        )
            ...
            if df[col].dtype == "object":
                ...
                if any(
                    any(pattern in str(val) for pattern in time_patterns) for val in sample_values
                ):
                    issues["time_strings"].append(
                        {"column": col, "sample_values": sample_values.tolist()}
                    )

            # Check for extreme outliers in numeric columns
            if (
                pd.api.types.is_numeric_dtype(df[col]) and df[col].dtype.kind not in "b"
            ):  # Exclude boolean
                non_null = df[col].dropna()
                if len(non_null) > 10:
                    try:
    ...
    def _analyze_modeling_suitability(self, df: pd.DataFrame) -> dict:
        ...
        for col in df.columns:
            ...
            if completeness["is_sufficient_for_modeling"] and pd.api.types.is_numeric_dtype(
                df[col]
            ):
                # Categorize by feature type
                if any(
                    time_keyword in col.lower() for time_keyword in ["time", "date", "duration"]
                ):
                    suitability["time_series"].append(col)
                elif any(
                    target_keyword in col.lower()
                    for target_keyword in ["score", "target", "outcome"]
                ):
    ...
    def _generate_recommendations(self) -> list:
        ...
        low_completeness = [
            col
            for col, data in self.analysis_results["completeness"].items()
    ...
    def print_summary(self):
        ...
        if info["date_range"]["column"]:
            logger.info(
                f"   • Date Range: {info['date_range']['start_date']} to {info['date_range']['end_date']}"
            )
        ...
        sufficient_cols = sum(
            1 for data in completeness.values() if data["is_sufficient_for_modeling"]
        )
        ...
        for category, features in categories.items():
            if features:
                ...
                logger.info(
                    f"   • {category.title()}: {len(features)} features ({sufficient_count} sufficient)"
                )
    ...
    def _generate_markdown_report(self) -> str:
        ...
        md_content.append(
            f"- **Columns with ≥50 non-null values:** {sufficient_cols}/{len(completeness)}"
        )
        md_content.append(
            f"- **Columns with ≥10% completeness:** {adequate_cols}/{len(completeness)}"
        )
        md_content.append("")
        ...
        md_content.append(
            f"- **Anomaly Detection:** {len(suitability['anomaly_detection'])} features"
        )
        md_content.append(f"- **Clustering:** {len(suitability['clustering'])} features")
        md_content.append(
            f"- **Predictive Modeling:** {len(suitability['predictive_modeling'])} features"
        )
