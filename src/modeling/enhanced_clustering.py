...
class EnhancedClusterer:
    ...
    def prepare_features(self, df: pd.DataFrame, feature_cols: Optional[List[str]] = None) -> Tuple[np.ndarray, List[str], pd.DataFrame]:
        ...
        if len(df_clean) < 10:
            raise ValueError(f"Not enough complete rows for clustering. Need at least 10, got {len(df_clean)}")
