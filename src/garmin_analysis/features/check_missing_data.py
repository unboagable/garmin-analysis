...
def audit_table_health(conn, tables_to_check):
    ...
    for table in tables_to_check:
        try:
            ...
            logger.warning("Error reading table '%s': %s", table, e)
            results.append({"table": table, "rows": 0, "status": "not found"})
    return pd.DataFrame(results)

def main(db_path="db/garmin.db", export_csv=True):
    if not os.path.exists(db_path):
        logger.error("Database file '%s' not found. Please run garmindb_cli.py or ensure the DB is in the project root.", db_path)
        return
    ...
    try:
        ...
        logger.warning("'files' table not found in the database.")
