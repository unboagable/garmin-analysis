...
def days_with_continuous_coverage(
    ...
    *,
    ...
    if df is None or df.empty or timestamp_col not in df.columns:
        logger.warning(
            "days_with_continuous_coverage received empty df or missing column '%s'", timestamp_col
        )
        return []
    ...
    for current_day, stamps in grouped:
        ...
        if len(diffs) > 0:
            ...
            internal_missing = safe_diffs.apply(lambda d: max(pd.Timedelta(0), d - max_gap)).sum()
        else:
...
def filter_master_by_days(
    ...
    *,
    ...
    if master_df is None or master_df.empty or day_col not in master_df.columns:
        logger.warning(
            "filter_master_by_days received invalid inputs (empty df or missing '%s')", day_col
        )
        return master_df
...
def calculate_daily_coverage_metrics(
    ...
    *,
    ...
    if df is None or df.empty or timestamp_col not in df.columns:
        logger.warning(
            "calculate_daily_coverage_metrics received empty df or missing column '%s'",
            timestamp_col,
        )
        return pd.DataFrame()
    ...
    for current_day, stamps in grouped:
        ...
        has_24h_coverage = (
            start_deficit == pd.Timedelta(0) and end_deficit == pd.Timedelta(0) and gap_count == 0
        )
    ...
    result_df["has_24h_coverage"] = result_df["has_24h_coverage"].astype(bool)
    return result_df
...
def filter_by_24h_coverage(
    ...
    *,
    ...
    if timeseries_df is None or timeseries_df.empty:
        logger.warning(
            "No monitoring_hr (heart rate) data available - cannot determine watch wear without HR data"
        )
        logger.warning(
            "Heart rate data is required to determine if watch is being worn (requires skin contact)"
        )
        return master_df
